  ...
  if (newFiles.data.files.length) {
    newFiles.data.files.forEach(function (file) {
      var nuevoArchivo = new Archivo({
        ...
        width:
          file.imageMediaMetadata?.width || file.videoMediaMetadata?.width || 0,
        height:
...
const getEtiquetasArchivoBBDD = async (req, res = response) => {
  ...
  try {
    let etiquetasArchivo = await Archivo.find({ id: idArchivo });

    let etiquetasArchivoArr = [];

    etiquetasArchivo.forEach((etiqueta) => {
      etiquetasArchivoArr.push(etiqueta);
    });

    console.log("Array de etiquetas obtenidas:", etiquetasArchivoArr);
