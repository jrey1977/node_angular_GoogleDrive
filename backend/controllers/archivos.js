...
graboNuevasCategorias = async (req, res = response) => {
    ...
    const foldersIdToName = new Object(
        foldersRes.data.files.reduce(
            (obj, o) => ((obj[o.id] = o.name), obj),
            {}
        )
    );
    ...
    try {
        ...
        const myAsyncLoopFunction = async (arrayCategorias) => {
            for (const idCategoria of arrayCategorias) {
                ...
                let etiquetaExisteBBDD = await Etiqueta.find({
                    id: idCategoria,
                });
                if (etiquetaExisteBBDD.length < 1) {
                    ...
                    var etiquetaGuardada = await etiqueta.save();
                    console.log(
                        "Etiqueta/categoria grabada con id",
                        idCategoria
                    );
                }
            }
            ...
            for (const file of newFiles.data.files) {
                if (file?.parents[0]) {
                    ...
                    var nuevoArchivoBueno = await Etiqueta.find({
                        id: file.parents[0],
                    });
                    if (nuevoArchivoBueno.length) {
                        ...
                        var nuevoArchivo = new Archivo({
                            ...
                            height:
                                ...
                                0,
                            durationMillis:
                                file.videoMediaMetadata?.durationMillis | 0,
                        });
                    ...
                    console.log(
                        "No graba el archivo porque parents es ",
                        file?.parents
                    );
                }
            }
...
const getEtiquetasArchivoBBDD = async (req, res = response) => {
    ...
    try {
        ...
        etiquetasArchivo.forEach(async (etiqueta, index) => {
            nombreEtiqueta = await Etiqueta.find({ _id: etiqueta }).select(
                "name"
            );
    ...
    if (primeraVez) {
        ...
        try {
            ...
            response.data.files.forEach(async function (file) {
                if (file?.parents) {
                    // Compruebo que esta imagen o video pertenezca a alguna de las etiquetas que son categoría
                    var archivoBueno = await Etiqueta.find({
                        id: file.parents[0],
                    });
                    if (archivoBueno.length) {
                        ...
                        if (
                            archivosGrabados % resultadosPorCadaUnoPorCiento ===
                            0
                        ) {
                            tantoPorciento++;
                            console.log(
                                "Tanto por ciento: " + tantoPorciento + "%"
                            );
                        }
                    }
                }
